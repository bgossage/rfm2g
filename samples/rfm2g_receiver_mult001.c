...

{
    ...
    do
    {
        ...
        /* Now read data from the other board from OFFSET_1_MULT001 */
        result = RFM2gRead( Handle, OFFSET_1_MULT001, (void *)buffer, BUFFER_SIZE*4 );
        if( result == RFM2G_SUCCESS )
        {
        ...
        if (verbose == RFM2G_TRUE)
        {
            ...
            for( i=0; i<BUFFER_SIZE; i++ )
            {
                printf( "Offset: 0x%X\tData: 0x%08X\n", OFFSET_1_MULT001 + i*4, buffer[i] );
            }
        }

        /* Now write the buffer into Reflective Memory starting at OFFSET_2_MULT001 */
        result = RFM2gWrite( Handle, OFFSET_2_MULT001, (void *)buffer, BUFFER_SIZE*4 );
        if( result == RFM2G_SUCCESS )
        {
