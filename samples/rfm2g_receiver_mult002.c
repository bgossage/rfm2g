...

{
    ...
    result = RFM2gEnableEvent( Handle, RFM2GEVENT_INTR3 );
    if( result != RFM2G_SUCCESS )
    {
        printf("Error: %s\n", RFM2gErrorMsg(result) );
        RFM2gClose( &Handle );
    ...
    do
    {
        ...
        /* Now read data from the other board from OFFSET_1_MULT002 */
        result = RFM2gRead( Handle, OFFSET_1_MULT002, (void *)buffer, BUFFER_SIZE*4 );
        if( result == RFM2G_SUCCESS )
        {
        ...
        if (verbose == RFM2G_TRUE)
        {
            ...
            for( i=0; i<BUFFER_SIZE; i++ )
            {
                printf( "Offset: 0x%X\tData: 0x%08X\n", OFFSET_1_MULT002 + i*4, buffer[i] );
            }
        }

        /* Now write the buffer into Reflective Memory starting at OFFSET_2_MULT002 */
        result = RFM2gWrite( Handle, OFFSET_2_MULT002, (void *)buffer, BUFFER_SIZE*4 );
        if( result == RFM2G_SUCCESS )
        {
        ...
        result = RFM2gSendEvent( Handle, otherNodeId, RFM2GEVENT_INTR4, 0);
        if( result == RFM2G_SUCCESS )
        {
